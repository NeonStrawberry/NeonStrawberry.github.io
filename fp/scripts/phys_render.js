...
const p_speed = 5;
const weapon_cooldown = 10; // Ticks to wait between weapon shots
const p_projectile_speed = 7;
const p_projectile_width = 10;
const p_wpncost = 5; // Power consumed by weapon
const max_power = 50;
...
function collides_with_enemy(pos, size) {
    for (let i = 0; i < enemies.length; i++) {
        if (Math.abs(pos[0] - enemies[i].pos[0]) <= (size / 2 + p_width / 2))
            if (Math.abs(pos[1] - enemies[i].pos[1]) <= (size / 2 + p_width / 2))
                return i;
    }
